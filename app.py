...
def register():
    if request.method == 'POST':
        ...
        new_user = {'id': user_id, 'username': username, 'password': password, 'liked_hotels': []}
...
def excursion(excursion_id):
    ...
    return render_template('hotel.html', excursion=excursion)
...
def get_comments():
    ...
    for hotel in hotels:
        user_hotel_comments = []
        comments = hotel.get('comments', [])
        for comment in comments:
            if comment["user_id"] == user_id:
                user_hotel_comments.append(comment)
        if user_hotel_comments:
            user_comments.append({"id": hotel["id"], "comments": user_hotel_comments})
...
def submit_comment(excursion_id):
    ...
    with open('hotels.json', 'w') as file:
        json.dump(hotels, file, indent=2)  # Write the modified excursion data back to the file
...
def like_hotel():
    ...
    try:
        hotel_id = request.json['excursionId']
    except KeyError:
        return jsonify({'success': False, 'message': 'Hotel ID not provided.'})
    ...
    user_id = request.cookies.get('user_id')
    for user in users:
        if str(user['id']) == user_id and  hotel_id not in user['liked_hotels']:
            # User found, update liked_hotels field
            user['liked_hotels'].append(int(hotel_id))
            break
...
def get_liked_hotels():
    ...
    for user in users:
        if str(user['id']) == user_id:
            liked_hotels = [excursion for excursion in hotels if excursion["id"] in user["liked_hotels"]]
            break
    
    return jsonify({'success': True, 'containers': liked_hotels})
